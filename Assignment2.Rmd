...
In this report we review the statistical methods used by Police Scotland to estimate blood alcohol concentration (BAC) at the time of driving in drink-driving investigations. Since BAC decreases over time, the level measured at the police station is lower than the level at the time of driving. To address this, forensic analysts “back-calculate’’ the earlier concentration using an elimination rate, denoted $\beta$ [@FSR2020].

The current forensic procedure applies a single fixed range of $\beta$ values to all individuals. In particular, the 2.5th percentile of the $\beta$-distribution is used to calculate a conservative back-calculated BAC: if this estimate exceeds the legal limit (0.47 g/kg), the individual is reported as over the limit. While straightforward and easy to communicate, this method does not account for differences between individuals, nor does it represent the uncertainty associated with back-calculation.

Using a dataset of 100 individuals [@thesis], we analysed how $\beta$ varies with personal characteristics. Weight and height were found to be strongly negatively associated with $\beta$, and males tended to show higher elimination rates than females. These findings indicate that $\beta$ is not constant across the population, and that a fixed threshold cannot reflect the true variability in elimination rates.

We compared several alternative modelling strategies.

Although a Gamma distribution provided a reasonable population-level fit and linear regression captured relationships with covariates, both approaches were limited in their ability to estimate the lower tail of the $\beta$-distribution - the region most relevant to forensic back-calculation. Quantile regression, by contrast, directly models the required conditional quantiles of $\beta$ and accommodates differences by sex, weight, and height. This makes it well-suited for personalised and uncertainty-aware BAC estimation.

We therefore recommend adopting a probabilistic reporting framework based on quantile regression. Instead of giving a single yes/no conclusion, forensic reports should provide:

-   the measured BAC and the elapsed time since driving;
-   the estimated BAC at the time of driving;
-   a 95% uncertainty interval reflecting statistical uncertainty;
-   the probability that the driver exceeded the legal limit.

Applying this framework to a case study (a 70-year-old female, 70 kg, 160 cm, measured BAC 0.15 g/kg after 2 hours), the current method concludes she was below the limit. In contrast, our quantile-regression-based analysis indicates a probability of approximately 94% that her BAC at the time of driving exceeded the legal limit, highlighting the importance of incorporating uncertainty and individual characteristics into the assessment.

We also considered situations where no blood sample is available and BAC must be estimated from the reported alcohol dose using Widmark’s equation. This calculation depends on both $\beta$ and the volume of distribution, $V_d$. We found that $V_d$ varies relatively little across individuals, but that $\beta$ and $V_d$ are negatively correlated, contrary to the current assumption that they are independent. Methods that treat them as independent and combine extreme values can produce BAC estimates that are not supported by the data. A more realistic approach is to base calculations on their observed joint behaviour and to report resulting BAC estimates with clear uncertainty intervals, rather than relying on fixed, extreme percentile combinations.

Overall, our findings support a shift towards probabilistic, model-based reporting that reflects individual characteristics, acknowledges dependence between key physiological parameters, and makes the associated uncertainty transparent to the courts.
...
The current forensic approach applies a single fixed range of $\beta$ values to all individuals. While simple, it ignores natural variability: characteristics such as age, sex, weight and height can all influence alcohol elimination, in part because they directly affect total body water (TBW), a key determinant of alcohol distribution in the body [@Reiter2020]. It is therefore important to assess whether $\beta$ can be better modelled using individual characteristics rather than a fixed population-wide range.
...
-   `Sex`: sex of the given individual;
...
-   as weight increases $\beta$ decreases;

-   as height increases $\beta$ decreases;

-   the majority of age data is scattered between 20-30, however there is a reasonably constant spread of values, suggesting age is less influential on $\beta$;

-   there is a clear difference in $\beta$ between sexes, with males typically obtaining higher values.

Table 1 evaluates the significance of correlation between each characteristic and $\beta$.
...
We can use Figures 5 and 6 to see that the shape is similar to Normal, Beta and Gamma distributions. Our method selection also relies on simplicity, as we do not want to over-fit our chosen distribution. The distribution fits are evaluated in Figure 7.
...
-   **Q-Q plot:** Lack-of-fit observed at distribution tails.
...
The density graph of the Gamma distribution across the whole population is shown in Figure 8.
...
Considering that sex is a factor variable, we can approximate the distributions of $\beta$ depending on whether the suspect is male or female.
...
While fitting a distribution by sex has allowed us to incorporate more information on the suspect, without overfitting to the data, sex is not the only characteristic given to the police. By creating a linear model, we can create a distribution for $\beta$ given a range of information about the suspect.
...
To analyse the fit of this model, we examine the residual ($\epsilon_i$) plots in Figure 10.
...
-   **Residual vs Fitted:** Random scatter of points around zero.

-   **Q-Q Residuals:** The ordered standardised residuals are plotted against quantiles of a standard normal. There is a lack of fit observed at distribution tails.

-   **Scale-Location:** We have random variation around the mean, so constant variance assumption seems reasonable.
...
\end{aligned}
$$To further demonstrate the difference between the linear and quantile regression, consider how we would find the 2.5th quantile of $\beta$ in both methods. For the linear model method, we have to find $E[\beta | x] + 2.5^\text{th}\text{ quantile of } \epsilon_i$. This assumes that the residuals are equally distributed across the whole population of $\beta$, which could be inaccurate. On the other hand, in quantile regression, we simply find the 2.5th quantile of the distribution of $\beta | x$. Not only does this not assume that residuals are equally distributed, but it is an intuitive approach, more aligned with the current methodology of Police Scotland, while improving the accuracy of results.

The subtle differences between the methods are shown in Figure 11.
...
This approach transparently communicates both what we know and what remains uncertain, allowing legal decision-makers to weigh the evidence appropriately. While we ultimately recommend incorporating quantile regression, we evaluate three approaches for generating these probability statements.

### Method 1: Gamma Distribution (Whole Population)

By fitting a Gamma distribution, we obtain a smooth representation of how $\beta$ varies in the data. The fitted distribution is $\beta \sim \text{Gamma}(\alpha = 31.95, \lambda = 173.71)$.

The 95% uncertainty interval for $C_0$ is constructed using the 2.5th and 97.5th percentiles of the Gamma distribution:$$
...
$$ \begin{eqnarray} \mathbb{P}(C_0 > x) &=& 1 - F_{\beta}\left(\frac{x - C_t}{t}\right), \end{eqnarray}$$where $x$ is the legal limit (0.47 g/kg) and $F_{\beta}$ is the cumulative distribution function of the Gamma distribution for $\beta$.
...
### Method 2: Sex-Specific Gamma Distributions
...
Depending on whether the suspect in question is male or female, we can create an uncertainty interval. For example, for a male suspect, we have

$$
C_0 \in (C_t + \beta_{male,0.025} \times t, C_t + \beta_{male,0.975} \times t). 
...
This model finds the $(\tau \times 100)^\text{th}$ quantile for $\beta$:$$
\begin{aligned}
\text{quantile regression model}: \quad 
\beta_{i,\tau} &= \alpha_{0,\tau}
+ \alpha_{1,\tau}\text{weight}_i
...
\epsilon_{i,\tau} &\overset{\mathrm{iid}}\sim \mathcal{N}(0, \sigma^2).
\end{aligned}
$$For a given individual $p$, we obtain personalized bounds from which we can create an uncertainty interval:

$$
C_0 \in (C_0 + \beta_{p,0.025} \times t, C_0 + \beta_{p,0.975} \times t).
$$

To find the probability that the individual is above the limit, we use a Monte Carlo approximation. Based on our exploratory data analysis, we deduce that the majority of $\beta$ values lie around a central value, so we choose a triangular distribution. We simulate 10000 samples from $\beta \sim \text{Triangular}(\beta_{p,0.025}, \beta_{p,0.5}, \beta_{p,0.975})$. Then we back-calculate $C_0$ and find the mean number of times that our simulated $C_0$ is greater than the legal limit:$$\begin{eqnarray} \mathbb{P}(C_0 > x) &=& \mathbb{P}(\text{# simulated samples of C}_0 \text{ s.t. C}_0 > x). \end{eqnarray}$$
...
## Benefits of Quantile Regression for the Justice System

The aim of our investigation for Police Scotland is to find the most accurate and plausible method for deciding if a suspect is above the legal drink-driving limit. The main advantage of quantile regression over the other methods described is that it takes in the full picture of the person in question. It incorporates significant physiological characteristics available to the police in a way that is practical to implement. In conjunction with the probabilistic framework, it offers several advantages:
...
To illustrate the differences between these methods, consider a 70-year-old female (weight: 70 kg, height: 160 cm) who provides a blood sample 2 hours after driving, with a measured BAC of $C_t$ = 0.15 g/kg. The legal limit is 0.47 g/kg. The police wish to charge her with the criminal offence of driving while over the legal limit.
...
The 95% uncertainty interval for $C_0$ is (0.40, 0.66) g/kg, and the probability that $C_0$ exceeded the legal limit is 76%.
...
The 95% uncertainty interval for $C_0$ is (0.44, 0.68) g/kh, and the probability that $C_0$ exceeded the legal limit is 91%.

*Verdict:* The lower bound of this uncertainty interval is very close to the legal limit, and the uncertainty interval found shows that there is significant evidence to indicate that the individual's true BAC at the time of driving exceeded 0.47 g/kg, with a high probability of 91%. This is a serious contrast to the current approach.

**4. Proposed Approach Using Quantile Regression**

We fit the model on the original dataset to find the 2.5th quantile, 50th quantile (median) and 97.5th quantile of $\beta | x$. Then we can predict given the suspect's specific characteristics (female, 70 kg, 160 cm, 70 years old), to find the 95% uncertainty interval is (0.43, 0.69) g/kg. The probability that $C_0$ exceeded the legal limit is 94%.
...
Figure 12 is a visual representation of the quantiles found above, also showing the probability of exceeding the legal limit.
...
Figures 13-17 help us visualise these relationships.
...
From Figure 13, we can see that the majority of $V_d$ values lie around 0.65, in a roughly normally distributed pattern, with the mean, 2.5th, and 97.5th quantiles highlighted in red. There appears to be one particular outlier, with a $V_d$ value of around 1.2, which may be worth investigating further.

Figures 14-17 highlight the following relationships:
...
It is also worth considering how the two rates are correlated, as independence is assumed between $\beta$ and $V_d$. Performing a correlation test between the two can help to deduce whether this assumption is upheld. Further, Figure 18 helps to visualise the relationship between $\beta$ and $V_d$.
...
Figure 18 further highlights this clear relationship between the two measures, and hence the independence assumption appears to be violated.
