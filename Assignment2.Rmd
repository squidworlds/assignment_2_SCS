...
-   **Parametric Distributions**: Gamma provided the most accurate fit to the observed $\beta$ values, outperforming normal and beta alternatives.

-   **Linear models**: Useful for incorporating continuous predictors, though residual diagnostics suggested violations of assumptions.

-   **Quantile Regression**: A more appropriate approach when estimating the lower tail of $\beta$, avoiding restrictive assumptions and allowing covariate-specific inference.

In our investigation, we propose replacing the current binary cutoff with a probabilistic and interval-based framework:

-   **Probabilistic Estimation**: Compute the probability that the BAC at the time of driving exceeded the legal limit, using Monte Carlo simulation or analytical integration of the fitted distribution.

-   **Interval Presentation**: A 95% credible interval for the back-calculated BAC is then provided, alongside the probability of exceeding the limit.

As a case study example, we found that under the current approach, an individual would be deemed not over the limit, while our suggested approaches provide respective probabilities of 76% and 75% that the individual was indeed over the limit - revealing a substantial likelihood that the individual was above the legal threshold, in contrast to the binary conclusion under current practice.
...
$$
C_0 = C_t + \beta t
$$to obtain a point estimate of the blood alcohol concentration at the time of driving. A binary decision is then made as to whether this estimated $C_0$ exceeds the legal limit.

To illustrate this, let us consider the following case study.
...
The current approach would take the 2.5th percentile of $\beta$ across the population, in our case we have $\beta$ = 0.126476. That would mean, in the current "back-calculation" approach, the forensics team would deduce their BAC at the time of driving to be

$$
C_0 = C_t + \beta t = 0.15 + 0.126476 \times2 = 0.402952<0.47.
$$

Due to the binary nature of this approach, since this back calculated value is below the legal limit, the conclusion is that the individual should not be prosecuted. While this approach is simple, it disregards uncertainty in $\beta$ and in the model used to derive it. Given we have highlighted the clear variation of $\beta$ between individuals, there is reason to query the valdity of this approach in borderline cases.
...
### Probabilistic (Simulation-Based) Estimation

Given that we have found evidence that $\beta$ may follow a Gamma distribution, that is,
...
Our other alternative, using quantile regression can too allow us to give a probabilistic approach to these cases. In this case...

SAIOA PLS WRITE IN HERE HOW THE PROBABILITY IS CALCULATED FROM QUANTILE REGRESSION.

This probability offers a more nuances summary than a simple 'above/below limit' approach. For example, if $\mathbb{P}(C_0 > x)$=0.88, the expert witness could state that there is an 88% probability that the suspect's blood alcohol concentration exceeded the legal limit at the time of driving.

### Interval Based Presentation
...
This provides a range of plausible values for $C_0$ given the uncertainty in $\beta$.

For the 70-year-old female described above, the fitted Gamma model for the elimination rate provides the following statistics.
...
We can see from table 2 that the 95% prediction/credible interval for this individual's BAC at the time of driving is $C_0 \in$ (0.40,0.66)g/kg, with an associated probability of being over the limit of approximately 76%.

Comparatively, quantile regression produces a 2.5% and 97.5% quantile for each possible combination of explanatory variables, so we have to interpret a confidence interval of possible elimination rates differently here. To give an indication of the range of beta under quantile regression, we can look at the mean at each quantile. That is,

SAIOA PLS GIVE A MATHEMATICAL EXPLANATION OF THIS HERE THANK YOU XXXX

The statistics associated with this approach, given the female in the case study, are given below.
...
From table 3, we see that the credible interval provided by the quantile regression approach for the individual's BAC at the time of driving is $C_0 \in$ (0.415,0.635)g/kg, with an associated probability of approaximately 75% that the individual was over the legal limit at time of driving.

Under the current forensic convention, the individual would be reported as below the limit. However, our uncertainty-based model reveals a substantial probability that her true level was above the limit, suggesting that a probabilistic or interval-based report would convey the evidence more transparently.

### Graphical Uncertainty Presentation
...
The following figure compares the approaches we have discussed above, that is

-   the current empirical (population) based approach, with a binary decision;

-   our suggested Gamma distribution approach, with a probabilistic decision approach, and corresponding confidence interval;

-   our suggested alternative quantile regression approach, also with a probabilistic decision approach, and corresponding confidence interval.
...
As illustrated in Figure , there is a great deal of overlap between our two recommended approaches, with both providing probabilities of above around 75% that the individual was above the legal limit at the time of driving. The narrower confidence interval provided by quantile regression indicates a decrease in uncertainty when this approach is taken.

Compared to the binary approach currently implemented by Police Scotland, these recommended approaches provide a measure of the uncertainty in such a procedure, and account for this in their probabilistic nature.

Thus, we recommend to Police Scotland that, when dealing with cases as illustrated in this case study, a probabilistic approach is incorporated to account for uncertainty in the back calculation approach. Especially when cases provide borderline estimates (as illustrated in this case study), it is crucial that these estimates are treated with the uncertainty that is associated with their derivation.
