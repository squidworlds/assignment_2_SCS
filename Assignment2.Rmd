...
In this statistical report, we investigate the techniques used by the forensic services at Police Scotland on blood alcohol computations in criminal cases associated with drink driving. Since breath tests used at the scene of a crime are not accurate enough for prosecution, back calculation techniques must be implemented once accurate blood alcohol data has been recorded to estimate whether the suspect was over the limit at the time of the crime.

The first method implemented by Police Scotland is the so-called $\beta$ elimination rate, which measures how quickly alcohol is eliminated from the blood stream. Using available data, a distribution for $\beta$ can be constructed, and the 2.5th and 97.5th percentiles of this distribution are used as the limits for this range. Currently, if an individual has a blood alcohol concentration above the legal limit, calculated using the lower 2.5th percentile of the $\beta$ distribution, the forensic team concludes that they are above the limit.

During our investigation, we found...

If it is too late for a blood or breath test to provide accurate results for the blood alcohol concentration at the time of the crime, eyewitness evidence measuring the quantity of alcohol consumed can be used in the Widmark's equation to calculate the blood alcohol concentration at a certain number of hours after drinking. This equation incorporates the eyewitness observed dose of alcohol consumed, the weight of the individual, and the so-called volume of distribution coefficient, "*a proportionality constant that relates the total amount of drug* (in our case alcohol) *in the body to the plasma concentration of the drug at a given time.*" (<https://www.ncbi.nlm.nih.gov/books/NBK545280/>). Currently, the forensics team at Police Scotland use the same approach when assessing the range of $V_d$, meaning they calculate the distributions of $\beta$ and $V_d$ separately and find their corresponding 2.5th and 97.5th percentiles, assuming the two measures are independent of one another.
...
sex_density_plot <- ggplot(data, aes(x = beta, color = sex, fill = sex)) +
  ...
  geom_vline(data = stats, aes(xintercept = q975, color = sex),
             linetype = "dotted", size = 1) +
  scale_color_manual(values = c("male" = "skyblue", "female" = "seagreen")) +
...
The 2.5% quantile in the gamma distribution of $\beta$ is 0.1257847. The Density graph of the gamma distribution across the whole population of Gamma is shown below to demonstrate fit.
...
```{r}
# Case Study Parameters (from task 2)
...
#    P(C0 > x) = P(beta > (x - Ct)/t)
beta_threshold <- (x - Ct) / t
p_over_limit <- 1 - pgamma(beta_threshold, shape = gamma_shape, rate = gamma_rate)

# 4. Monte Carlo simulation (for illustration if can't use analytical method)
set.seed(123)
n_sim <- 1000
...
# Output neatly
cat("95% credible interval for C0: (", round(C0_lower,3), ",", round(C0_upper,3), ")\n")
cat("Probability(C0 > limit) [analytical] =", round(p_over_limit,3), "\n")
cat("Probability(C0 > limit) [simulation] =", round(p_over_limit_sim,3), "\n")
cat("Mean C0 =", round(C0_mean,3), ", SD =", round(C0_sd,3), "\n")
...
*Not sure whether to put this bit in task 2 analysis!:*
...
#### **3. Graphical 'Uncertainty Band' Presentation**

Plot $C_0 = C_t + \beta t$ versus $\beta$, and overlay:

-   the estimated Gamma PDF for \beta;
-   a vertical line at the legal limit;
-   shading for the proportion of the $\beta$ distribution that yields $C_0 > x$

This visually communicates both uncertainty in $\beta$, and how close the case is to the threshold.
...
ggplot(data, aes(x = Vd)) +
  ...
  labs(title = "Figure 5: Histogram of Volume of Distribution (Vd)",
       x = "Vd (L/kg)", y = "Count")
```
