...
lm_model <- lm(beta60 ~ weight + age + height + sex, data = data)
summary(lm_model)
...
sqrt_lm_model <- lm(sqrt(-1 * beta60) ~ weight + age + height + sex, data = data)
summary(sqrt_lm_model)
...
The above method is sensible, as it finds a distribution for $\beta$ given a range of information about the suspect, whether this information is in the form of discrete or continuous form. This is more useful than the method that finds a different beta distribution for males and females, as it is not possible to do the same for continuous information such as age. Therefore the previous methods don't make the most of all the information given. Even the linear model method above has some flaws, as it's aim is to find the conditional mean function, $E(\beta | x)$, where $x$ represents the explanatory variables of the model. However, the aim of our analysis is to find the 2.5% quantile of $\beta$. Therefore it is more accurate to use quantile regression.

```{r quantile_reg, echo = FALSE, message = FALSE, warning=FALSE, fig.width=20, fig.height=10, fig.align='center'}

rq_model <- rq(beta60 ~ weight + age + height + sex, data = data, tau = 0.025)
summary(rq_model)
...
# empirical quantiles for lm
q025 <- quantile(residuals(lm_model), probs = 0.025)
pred_data$lm_pred <- predict(lm_model)
pred_data$lm_q025_empirical <- pred_data$lm_pred + q025
...
plot_data <- pred_data %>%
  tidyr::pivot_longer(cols = c(lm_q025_empirical, rq_pred),
                      names_to = "model",
                      values_to = "predicted_beta")

# Plot
ggplot(plot_data, aes(x = beta60, y = predicted_beta, color = model)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(
    title = expression("Comparing Models for "*beta),
    x = expression("Actual "*beta),
    y = expression("Predicted "*beta),
    color = "Model Type"
  ) +
  scale_color_manual(values = c("lm_q025_empirical" = "firebrick", "rq_pred" = "forestgreen"),
...
A 70 year old female (weight: 70kg, height: 160cm) is arrested after being stopped by the police while
driving. She provides a blood sample to the police 2 hours after her arrest which gives a reading of
Ct = 0.15g/kg (i.e. grams of alcohol per kilogram of blood). The legal limit is x = 0.47g/kg. The
police wish to charge her with the criminal offence of driving while over the legal limit.
...
C0_gamma_abs <- Ct + abs(gamma_quantiles) * t

# gamma fitted to males
m_gamma_quantiles <- -qgamma(c(0.975, 0.5, 0.025), m_gamma_shape, m_gamma_rate)
C0_gamma_m <- Ct + m_gamma_quantiles * t
C0_gamma_abs_m <- Ct + abs(m_gamma_quantiles) * t

# gamma fitted to females
f_gamma_quantiles <- -qgamma(c(0.975, 0.5, 0.025), f_gamma_shape, f_gamma_rate)
C0_gamma_f <- Ct + f_gamma_quantiles * t
C0_gamma_abs_f <- Ct + abs(f_gamma_quantiles) * t
...
results_table <- tibble(
  Approach = c("Population (empirical quantiles)", "Model-based (predicted)", "Gamma Fitted", "Male Gamma Fitted", "Female Gamma Fitted"),
  Lower_2.5 = c(C0_pop_abs[1], C0_model_abs[2], C0_gamma_abs[1], C0_gamma_abs_m[1], C0_gamma_abs_f[1]),  # note ordering: lwr=2nd col in pred
  Central   = c(C0_pop_abs[2], C0_model_abs[1], C0_gamma_abs[2], C0_gamma_abs_m[2], C0_gamma_abs_f[2]),
  Upper_97.5 = c(C0_pop_abs[3], C0_model_abs[3], C0_gamma_abs[3], C0_gamma_abs_m[3], C0_gamma_abs_f[3])
)
