...
In this report we evaluate the statistical methods used by Police Scotland to estimate blood alcohol concentration (BAC) at the time of driving in drink-driving investigations. Because BAC decreases over time, the level measured at the police station is lower than the level at the time of driving. To address this, forensic analysts “back-calculate’’ the earlier concentration using an elimination rate, denoted $\beta$ [@FSR2020].
...
-   a 95% prediction interval reflecting statistical uncertainty;
...
Therefore we are finding the quantiles of $\beta$ in the following model [@quantile_reg], where $\alpha_{j,\tau}$ represent the coefficients associated with the $(\tau$ x $100)^{\text{th}}$ quantile for $j \in \{0, 1, 2, 3\}$, to estimate $\beta_{i,\tau}$, where $i \in \{1, \dots, 100\}$:$$
\begin{aligned}
...
\end{aligned}
$$To further demonstrate the difference between the linear and quantile regression, consider how we would find the 2.5% quantile of $\beta$ in both methods. For the linear model method, we have to find $E[\beta | x] + 2.5\text{% quantile of } \epsilon_i$. This assumes that the residuals are equally distributed across the whole population of $\beta$, which could be inaccurate. On the other hand, in quantile regression, $\beta_{i,0.025}$ corresponds to the 2.5% quantile of the distribution of $\beta | x$. Not only does this not assume that residuals are equally distributed, but it is an intuitive approach, more aligned with the current methodology of Police Scotland, while improving the accuracy of results.
...
This model finds the $(\tau$ x $100)^{\text{th}}$ quantile for $\beta$:$$
\begin{aligned}
\text{quantile regression model}: \quad 
\beta_{i,\tau} &= \alpha_{0,\tau}
+ \alpha_{1,\tau}\text{weight}_i
...
geom_text(
  ...
  scale_colour_manual(values = c("Current (Fixed β = 0.126)" = "grey60",
                      ...
                      name = "Method") +
  ggtitle(expression("Figure 13: Comparison of Prediction Intervals for Reporting Frameworks for "*C[0]*" in Case Study"))
